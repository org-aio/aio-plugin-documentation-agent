...
const formData = reactive<RoleDataPermissionFormData>({
  id: undefined,
  name: '',
  code: '',
  dataScope: undefined,
...
const open = async (row: RoleApi.RoleVO) => {
  ...
  try {
    ...
    await nextTick()
    formData.dataScopeDeptIds.forEach((deptId) => {
...
const submitForm = async () => {
  const { id: roleId, dataScope } = formData
  if (roleId === undefined || dataScope === undefined) {
    message.error('请选择角色及权限范围后再提交')
    return
  }
  let dataScopeDeptIds: number[] = []
  if (dataScope === SystemDataScopeEnum.DEPT_CUSTOM) {
    if (!treeRef.value) {
      message.error('部门范围尚未加载，请稍后再试')
      return
    }
    dataScopeDeptIds = treeRef.value
