...
interface DictDataFormData extends Omit<DictDataApi.DictDataSaveVO, 'sort'> {
  sort?: number
}
...
const formData = ref<DictDataFormData>({
  id: undefined,
  sort: undefined,
  label: '',
  value: '',
...
const submitForm = async () => {
  ...
  const sort = formData.value.sort
  if (!valid || sort === undefined) {
    return
  }
  // 提交请求
  formLoading.value = true
  try {
    const data: DictDataApi.DictDataSaveVO = { ...formData.value, sort }
